...
public class ConsentManagerImpl implements ConsentManager {
    ...
    /**
     * This API is used to update a Purpose.
     *
     * @param purpose Purpose element which needed to be updated.
     * @return Return updated purpose element.
     * @throws ConsentManagementException Consent Management Exception.
     */
    public Purpose updatePurpose(Purpose purpose) throws ConsentManagementException {

        validateInputParametersForPurposeUpdate(purpose);
        Purpose purposeResponse = getPurposeDAO(purposeDAOs).updatePurpose(purpose);
        return populatePiiCategories(purposeResponse);
    }
    ...
    public Purpose getPurpose(String purposeId, int version) throws ConsentManagementException {
        ...
        Purpose purpose = getPurposeDAO(purposeDAOs).getPurpose(purposeId, version, getTenantIdFromCarbonContext());
        if (purpose == null) {
            if (log.isDebugEnabled()) {
                log.debug("No purpose found for the Id: " + purposeId);
            }
    ...
    public Purpose getPurpose(String purposeId) throws ConsentManagementException {
        ...
        Purpose purpose = getPurposeDAO(purposeDAOs).getPurposeById(purposeId, getTenantIdFromCarbonContext());
        if (purpose == null) {
            if (log.isDebugEnabled()) {
                log.debug("No purpose found for the Id: " + purposeId);
            }
    ...
    public boolean isPurposeExists(String purposeId) throws ConsentManagementException {
        ...
        return getPurposeFromId(purposeId) != null;
    }
    ...
    private void validateInputParametersForPurposeUpdate(Purpose purpose) throws ConsentManagementException {

        String purposeId = purpose.getPurposeId();
        if (!isPurposeExists(purposeId)) {
            if (log.isDebugEnabled()) {
                log.debug("A purpose doesn't exists with ID: " + purposeId);
            }
            throw ConsentUtils.handleClientException(ERROR_CODE_PURPOSE_ID_INVALID, String.valueOf(
                    purposeId));
        }
        ...
        if (isBlank(purpose.getGroup())) {
            if (log.isDebugEnabled()) {
                log.debug("Purpose group is empty for: " + purposeId);
            }
            throw handleClientException(ERROR_CODE_PURPOSE_GROUP_REQUIRED, null);
        }
        ...
        if (isBlank(purpose.getGroupType())) {
            if (log.isDebugEnabled()) {
                log.debug("Purpose group type is empty for: " + purposeId);
            }
            throw handleClientException(ERROR_CODE_PURPOSE_GROUP_TYPE_REQUIRED, null);
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("Purpose update request validation success: " + purposeId);
        }
    }
    ...
    private Purpose getPurposeFromId(String purposeId) throws ConsentManagementException {

        return getPurposeDAO(purposeDAOs).getPurposeById(purposeId, getTenantIdFromCarbonContext());
    }
